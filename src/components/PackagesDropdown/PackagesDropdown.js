...
export default class PackagesDropdown extends React.Component {
  render() {
    ...
    for (var i = 0; i < this.props.packages.length; i++) {
      if (
        this.props.packages[i].badgeCount &&
        this.props.packages[i].id !== PackageInformation.packageId
      ) {
    ...
    return (
      <li className="nav-item dropdown" style={{ display: 'flex' }}>
        <IndexLink
          ...
          id="packageDropDownLink"
          data-toggle="dropdown"
          aria-haspopup="true"
          aria-expanded="false">
          <i className={packageIcon} aria-hidden="true" /> {PackageInformation.name}
          <span className="caret" />
          {badgeCount > 0 ? <span className="badge package-badge">{badgeCount}</span> : undefined}
        </IndexLink>
        <div
          className="dropdown-menu"
          aria-labelledby="packageDropDownLink"
          style={{
            width: '99vw',
            position: 'absolute',
            left: '-15px',
          ...
          {this.props.packages.map((p, index) => (
            <PackageTile key={`${index}`} package={p} />
          ))}
        </div>
      </li>
    )
