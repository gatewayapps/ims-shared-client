    ...
    return (
      <li className='nav-item dropdown' style={{ display: 'flex' }}>
        <IndexLink to='/' className='navbar-brand nav-link dropdown-toggle' id='packageDropDownLink' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>
          ...
          {
            badgeCount > 0
            ? <span className='badge package-badge'>{badgeCount}</span>
            : undefined
          }
        </IndexLink>
        <div className='dropdown-menu' aria-labelledby='packageDropDownLink' style={{ width: '99vw', position: 'absolute', left: '-15px', maxWidth:'410px', padding: '3px' }} >
