...
export class UserAutosuggestItem extends React.Component {
  render() {
    return (
      <div className="ims-user-autosuggest-item__container">
        <div className="ims-user-autosuggest-item__profile-image">{this.renderProfileImage()}</div>
  ...
  renderProfileImage() {
    if (this.props.user.profileImageUrl) {
      return (
        <img className="img-circle" height={30} width={30} src={this.props.user.profileImageUrl} />
      )
    } else {
      return (
        <span>
