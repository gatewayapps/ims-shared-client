...
export class FeedbackButton extends React.Component {
        ...
        <div className="form-group">
          ...
          <div className="col-sm-10">
            <p className="form-control-static">
              {PackageInformation.name} ({PackageInformation.packageId})
            </p>
          </div>
        </div>
        <div className="form-group">
          ...
          <div className="col-sm-10">
            <p className="form-control-static">
              {PackageInformation.version} ({PackageInformation.commit})
            </p>
            {PackageInformation.coreVersion && (
              <p className="form-control-static">Core: {PackageInformation.coreVersion}</p>
            )}
            {PackageInformation.clientVersion && (
              <p className="form-control-static">Client: {PackageInformation.clientVersion}</p>
            )}
          </div>
        </div>
      </form>
  ...
  render() {
    ...
    const sendIcon = this.state.sending ? 'fa fa-fw fa-circle-o-notch fa-spin' : 'fa fa-fw fa-send'
    ...
    return (
      <span>
        <button
          ...
          <i className={`fa fa-fw ${this.props.icon || 'fa-question-circle'}`} />
          <span className="d-md-none ml-1">Feedback</span>
        </button>
        <Modal zIndex={this.props.zIndex} isOpen={this.state.show} toggle={() => this._onHide()}>
          <ModalHeader closeButton>Send Feedback</ModalHeader>
