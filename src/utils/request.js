...
export function prepareRequest(store, tokensPath, packagesPath) {
  if (store && store.getState && store.dispatch) {
    ...
    throw new TypeError(
      'Invalid "store" should be an instance of a redux store'
    );
  }
  if (typeof packagesPath === "string") {
    ...
    throw new TypeError(
      'Invalid "packagesPath" should be either a string or an array'
    );
  }
  if (typeof tokensPath === "string") {
    ...
    throw new TypeError(
      'Invalid "tokensPath" should be either a string or an array'
    );
  }
...
export function download(url, options) {
  try {
    ...
    if (opts.authenticate) {
      return makeAuthenticatedDownload(
        url,
        opts.requestOptions,
        opts.defaultFileName
      );
    } else {
      return makeUnauthentiatedDownload(
        url,
        opts.requestOptions,
        opts.defaultFileName
      );
    ...
    if (token) {
      if (token.expires < moment().add(1, "minute").unix()) {
        return refreshAccessToken().then(() =>
          getAccessTokenForPackage(packageId)
        );
      } else {
...
function scheduleRefreshAccessToken() {
  ...
  if (tokens) {
    ...
    REFRESH_ACCESS_TOKEN_TIMEOUT = setTimeout(
  ...
  if (requestOptions.packageId) {
    ...
    if (pkg && pkg.packageUrl) {
      return getAccessTokenForPackage(pkg.packageId).then((accessToken) => {
        ...
        return makeRequest(
          combineUrlParts(pkg.packageUrl, url),
          requestOptions
        );
      });
    } else {
      throw new Error(
        `Package ${
          requestOptions.packageId
        } was not found in ${packagesPathKey.join(
          "."
...
function makeUnauthenticatedRequest(url, requestOptions) {
  requestOptions.headers = HeaderUtils.createRequestHeader(
    PackageInformation.packageId
  );
  return makeRequest(url, requestOptions);
}
...
function makeDownloadRequest(url, requestOptions, defaultFileName) {
  return fetch(url, requestOptions).then(
    parseDownloadResponse(defaultFileName)
  );
}
...
export function makeRefreshAccessTokenRequest() {
  ...
  return fetch(
    `${window.__HUB_URL__}/users/${accessTokenEndpoint}`,
    refreshOptions
  ).then(parseResponse);
}
...
export function refreshAccessToken(scheduleRefresh) {
  return makeRefreshAccessTokenRequest()
      ...
      if (response.success === true) {
        if (response.results) {
          for (var i = 0; i < response.results.length; i++) {
            if (
              response.results[i].packageId === PackageInformation.packageId
            ) {
              if (
                !response.results[i].accessToken ||
          ...
          storeInstance.dispatch(
            updateAccessToken(response.accessToken, response.expires)
          );
        }
    ...
    .catch((error) => {
      refreshAttempts++;
      console.error(
        `Failed refreshing access token attempt ${refreshAttempts}`,
        error
      );
      setTimeout(
        () => refreshAccessToken(true),
        REFRESH_ATTEMPT_DELAY * refreshAttempts
      );
    });
}
...
function prepareOptions(options = {}) {
  ...
  if (!opts.requestOptions) {
    ...
    opts.requestOptions = Object.assign(
      {},
      requestOptions,
      opts.requestOptions
    );
...
function goToLogin(loginUrl) {
  ...
  return {
    success: false,
    message: "Invalid refreshToken redirecting to login",
  };
}
