...
export function loadInitialState(opts) {
  return loadInitialStateFromLocalStorage().then((initialState) => {
    if (initialState) {
      return { fromLocalStorage: true, initialState: initialState }
    } else {
      return loadInitialStateFromServer(opts.url).then((initialState) => {
...
function loadInitialStateFromLocalStorage() {
  return getItems([INITIAL_STATE_STORAGE_KEY, SECURITY_STORAGE_KEY]).then((storageData) => {
    if (
  ...
  if (response.results) {
    ...
    for (var i = 0; i < response.results.length; i++) {
      ...
      if (response.results[i].packageId === PackageInformation.packageId) {
        securityObject = createSecurityState(
...
export function loadInitialStateFromServer(url) {
  return fetchInitialStateFromServer(url).then((serverResponse) => {
    if (
      serverResponse.success !== true ||
      !_.isObjectLike(serverResponse.result) ||
      _.keys(serverResponse.result).length === 0
    ) {
      return undefined
    }

    return getItem(SECURITY_STORAGE_KEY).then((securityObj) => {
      if (!_.isObjectLike(securityObj) || !_.isObjectLike(securityObj.currentUser)) {
        return makeRefreshAccessTokenRequest().then((securityResponse) => {
          let security
          ...
          return setItems(storageData).then(() => {
        ...
        return setItem(INITIAL_STATE_STORAGE_KEY, serverResponse.result).then(
...
export function getPackageBadgeCount(pkgDef) {
  if (pkgDef.badgeUrl) {
    // could just do direct check here to see if it fixes issue
    return request(pkgDef.badgeUrl)
      .then((result) => {
